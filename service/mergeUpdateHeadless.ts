...
function pruneTilesetFile(
  ...
  if (tileset.root) {
    ...
    // so hole filling can borrow coarse base geometry too.
    ...
    try {
      ...
      conversionProcess.on('close', (code) => {
        ...
        if (isCancelled) {
          ...
          reject(new Error(`转换进程异常退出: ${code}`))
        }
      })
      ...
      conversionProcess.on('error', (err) => {
        ...
        const message = err.message.includes('ENOENT')
          ? `未找到转换程序: ${exePath},请检查安装目录`
          : `转换程序启动失败: ${err.message}`
        reject(new Error(message))
      })
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err)
      reject(new Error(`转换失败: ${message}`))
    }
  })
}
...
function applyAggregationWithLog(
  ...
  emitStdout(params, '正在聚合瓦片(合并细碎 tile,保留 LOD)...\n')
  try {
    ...
    const message = err instanceof Error ? err.message : String(err)
    emitStderr(params, `聚合失败: ${message}\n`)
  }
}
