...
function auditTileset(tilesetPath, parentTransform, skipRoot, depth, out, label){
  ...
  const walk=(tile, pt, d, skipOwn=false)=>{
    ...
    const childRects=[];
    if(isExt&&uri){ const ep=path.resolve(dir,uri); if(fs.existsSync(ep)){ auditTileset(ep,tt,false,d+1,out,label); childRects.push(bounds); } }
    if(tile.children){ for(const c of tile.children){ const res=walk(c,tt,d+1,false); if(res) childRects.push(res); } }
    ...
    if(bounds && hasChildren && !hasContent){
      ...
      if(ratio < 0.9999){
        out.push({label, depth:d, uri:uri||'(placeholder)', ratio:+ratio.toFixed(4), box: [Math.round(bounds.minX),Math.round(bounds.minY),Math.round(bounds.maxX),Math.round(bounds.maxY)]});
      }
    }
    return bounds;
  };
...
const out=[];
auditTileset(path.join(dir,'tileset.json'), undefined, true, 0, out, path.basename(dir));
console.log('空洞候选（父无内容且子未全覆盖）:', out.length);
const byDepth={};
for(const h of out){ byDepth[h.depth]=(byDepth[h.depth]||0)+1; }
console.log('按层级分布:', JSON.stringify(byDepth));
console.log('前 40 个:', JSON.stringify(out.slice(0,40), null, 1));
